...
def save_chat_history():
    try:
        ...
        all_chats.append({"timestamp": datetime.now().isoformat(), "chat": history})
    ...
    try:
        ...
        if os.path.exists(PREFS_PATH):
            with open(PREFS_PATH, 'r') as file:
                ...
                prefs = current_data if isinstance(current_data, list) else [current_data]
        prefs.append({"region": region, "topic": topic, "timestamp": datetime.now().isoformat()})
...
with st.sidebar:
    ...
    if st.button("📌 Save Preferences", key="save_prefs_btn"):
        if st.session_state.get("region") and st.session_state.get("topic"):
            save_user_preferences(st.session_state.region, st.session_state.topic)
        else:
    ...
    if os.path.exists(PREFS_PATH):
        with open(PREFS_PATH, 'r') as f:
            ...
            st.download_button("⬇️ Download Preferences JSON", prefs_json, file_name="user_preferences.json")
    ...
    if os.path.exists(CHAT_LOG_PATH):
        with open(CHAT_LOG_PATH, 'r') as f:
            ...
            st.download_button("⬇️ Download Chat History JSON", chat_json, file_name="chat_history.json")


# Chat logic and rendering
if st.session_state.stage == "welcome":
    add_bot_message_once("welcome_msg", "👋 Welcome to Al Jazeera News Assistant! How would you like to explore the news today?")
    ...
    if results.empty:
        add_bot_message_once("no_articles", "❌ No articles found for this topic.")
    else:
    ...
    if col1.button("🔄 Restart Chat", key="restart_chat_button"):
        ...
        st.rerun()
    if col2.button("➕ New Search", key="new_search_button"):
        st.session_state.stage = "region"
        st.session_state.bot_flags = set()
        st.rerun()
    if col3.button("⬅️ Back to Mode Selection", key="back_to_mode_button"):
        st.session_state.stage = "welcome"
        st.session_state.bot_flags = set()
        st.rerun()
